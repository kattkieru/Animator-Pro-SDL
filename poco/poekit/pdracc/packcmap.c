 ...
 ****************************************************************************/
static Errcode find_newc(int usedc, int freec, Rgb3* ctab, int threshold, Rgb3* lsource,
						 long lscount, USHORT* curthresh)
{
	Errcode err;
	int closestix;
	long new;
	...
	poeprintf("Packing %6ld colors to %4d threshold %4d", lscount - usedc, freec, threshold);
 ...
 ****************************************************************************/
Errcode fpack_ctable(Rgb3* source, /* Source - array scount long of rgb triples */
...
{
	int threshold; /* how far away is the closest color before put in this one? */
...
FINDHI:
	...
	*curthresh = 0;
	newused = find_newc(usedc, dcount - usedc, dest, threshold, source, scount, curthresh);
	...
	/* Keep doing thresholds until we run out of colors. */
	hiwater = threshold;
	reduction_factor = 2;
	while (threshold >= 3) {
		newused = find_newc(usedc, dcount - usedc, dest, threshold, source, scount, curthresh);
