...
static Errcode load_hostlibs(char* name, Rexlib* rex_header, Hostlib* first_hl, Libhead** hostlibs)
{
	USHORT type;
	Libhead* hosthead;
	Hostlib *prev_hl, *hl;
	...
	while (first_hl) {
		...
		if (hosthead == NULL) {
			/* no more host libraries see if what rexlib still wants is optional
										 ...
										 * must be non NULL */
{
	...
	*prl = NULL;
	...
	err = pj_rex_load(name, &entry, prl);
	if (err < Success) {
		return err;
	}
	...
	if (id_string != NULL) {
		if (rl->id_string == NULL || 0 != strcmp(rl->id_string, id_string)) {
	...
	err = load_hostlibs(name, rl, firstlib, hostlibs);
	if (err < Success) {
		goto error;
	}
	goto done;
