...
Errcode pj_open_rex(char* path, Jfile* pfile, EXP_HDR* hdrsp)
{
	...
	hdrsp->exp_ldisize =
		((ohdrs.exe_size) - 1) * REX_BLK_SIZE + (ohdrs.exe_szrem) - (hdrsp->exp_ldimg);
...
static Errcode read_fixup_code(Jfile file, EXP_HDR* hdrsp, void** pprex, void** pplib)
{
...
#define RLIB ((Rexlib_entry*)rel_buf)
	...
	if (pplib) {
		/* we are expecting a pj style rex library
		 * so verify the header data */
		if ((err = pj_readoset(file, RLIB, hdrsp->exp_ldimg, sizeof(Rexlib_entry))) < Success) {
			goto error;
		}
		if (RLIB->magic != PJREX_MAGIC || RLIB->magic2 != PJREX_MAGIC2) {
			err = Err_not_rexlib;
			goto error;
		}

		if (RLIB->version != PJREX_BETAVERS && RLIB->data_end > hdrsp->exp_ldisize &&
	...
	err = pj_readoset(file, load_addr, hdrsp->exp_ldimg, hdrsp->exp_ldisize);
	if (err < Success) {
		goto error;
	}
	...
	clear_mem(OPTR(load_addr, hdrsp->exp_ldisize), load_size - hdrsp->exp_ldisize);

	err = pj_seek(file, hdrsp->exp_rel, JSEEK_START);
	if (err < 0) {
		goto error;
	}
	...
	while (hdrsp->exp_relsize > 0) {
		...
		while (rel_entry < rel_max) {
			if (*rel_entry & REL32) {
				*((ULONG*)OPTR(load_addr, (*rel_entry & (~REL32)))) += *((ULONG*)&load_addr);
			} else {
				*((USHORT*)OPTR(load_addr, *rel_entry)) += *((ULONG*)&load_addr);
			}
			++rel_entry;
		}
	} /* end for loop */
...
Errcode pj_rex_load(char* path, void** rexpp, void** prexlib)
{
	...
	*rexpp = NULL; /* in case of error out */
	...
	Errcode err = pj_open_rex(path, &file, &hdrs);
	if (err < Success) {
		goto error;
	}
