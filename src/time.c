...
static Errcode delete_middle_frames(int start, int frames)
{
	...
	err = write_flx_frame(&flix, start, cbuf);
...
static void delete_first_frames(int frames)
{
	...
	err = write_flx_frame(&flix, 0, cbuf);
	if (err < 0) {
		goto error;
	}
	...
	pj_fli_comp_cel(cbuf, undof, vb.pencel, COMP_DELTA_FRAME, flix.comp_type);
	err = write_flx_frame(&flix, flix.hdr.frame_count, cbuf);
	if (err < 0) {
		goto error;
	}
...
static Errcode make_frames(int frames)
{
	...
	flx_clear_olays();
	if (frames > flix.hdr.frame_count) {
		err = insert_frames(frames - flix.hdr.frame_count, flix.hdr.frame_count - 1);
	} else {
...
static Errcode expand_cur_flx(register int new)
{
	...
	new_flx = pj_zalloc(new * sizeof(Flx));
	if (new_flx == NULL) {
		err = Err_no_memory;
		goto error;
	}
					  ...
					  int where) /* frame index to insert frames after */
{
	...
	err = expand_cur_flx(count + flix.hdr.frame_count);
	if (err < 0) {
		goto error;
	}
	...
	back_copy_mem(open_part, open_part + count, (flix.hdr.frame_count + 1 - where) * sizeof(Flx));
